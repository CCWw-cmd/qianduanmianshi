...
![](https://s.poetries.top/gitee/2020/08/76.png)
...
> 利用ngnix跨域的关键就是在配置文件中设置server项，然后设置其中的location属性，proxy\_pass：需要代理的服务器地址，`add_header`：给响应报文中添加首部字段，例如`Access-Control-Allow-Origin`设置为`*`，即允许所有的请求源请求
